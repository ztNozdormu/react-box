...
contract FundingFactory {
    ...
    Funding[] allFundings;     // 1. 所有的众筹合约集合

    mapping(address => Funding[]) creatorFundings;  //创建人的合约集合
    ...
    function createFunding(string memory _name, uint _targetMoney, uint _supportMoney, uint _duration) public {
       ...
       // address funding = new Funding(_name, _targetMoney, _supportMoney, _duration, msg.sender, supportorFundings);
        Funding funding = new Funding(_name, _targetMoney, _supportMoney, _duration, msg.sender, supportorFundings);
    ...
    //返回当前账户所创建所有的合约
    function getCreatorFundings() public view returns (Funding[] memory) {
        return creatorFundings[msg.sender];
    }

    //获取回当前账户所的参与的合约的集合
    function getSupportorFunding() public view returns (Funding[] memory) {
...
contract SupportorFundingContract {
    ...
    mapping(address => Funding[]) supportorFundingsMap;

    function setFunding(address _supptor, Funding _funding) public {
        supportorFundingsMap[_supptor].push(_funding);
    }

    function getFundings(address _supptor) public view returns (Funding[] memory) {
...
contract Funding {
    ...
    constructor( string memory _projectName, uint256 _targetMoney, uint256 _supportMoney, uint256 _duration, address _creator, SupportorFundingContract _supportorFundings) public {
        // manager = msg.sender;
        manager = _creator;
    ...
    function refund() onlyManager public {
        for (uint256 i = 0; i < investors.length; i++) {
            address(uint160(investors[i])).transfer(supportMoney);
        }
    ...
    function createRequest(string memory _purpose, uint256 _cost, address _seller) onlyManager public {
    ...
    function finalizeRequest(uint256 i) onlyManager public {
        ...
        // 2. 执行转账
        address(uint160(req.seller)).transfer(req.cost);
    ...
    //
    function getRequestByIndex(uint256 i) public view returns (string memory, uint256, address, uint256, RequstStatus) {
