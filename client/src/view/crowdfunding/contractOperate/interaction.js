...
const contract =  contractService.InitContractInstarces();
const web3 = contractService.InitWeb3Instance();
...
let getFundingDetails = async (index) => {
    ...
    const fundingFactoryInstance =  await contract.fundingFactory;
    ...
    let detailsPromises = currentFundings.map(function (fundingAddress) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                const fundingInstance =  await contract.funding;
                // 动态指定新创建合约的地址
                fundingInstance.options.address = fundingAddress
                ...
                let investorCount = await fundingInstance.methods.getInvestorsCount().call()
...
const approveRequest = (fundingAddress, index) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const result = await fundingInstance.methods.approveRequest(index).send({
                from: accounts[0],
            });
...
const finalizeRequest = (fundingAddress, index) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const result = await fundingInstance.methods.finalizeRequest(index).send({
                from: accounts[0],
            });
